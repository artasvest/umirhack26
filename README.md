...
| Менеджер | Список с фильтром по статусу, карточка: резюме, скрипт звонка (заглушка), таблица ответов, статусы, `tel:`, заметки, заглушка голоса (Whisper позже), напоминание (сохранение в БД) |
...
2. Whisper для `POST /api/leads/:id/notes/voice`.
3. Планировщик напоминаний + сообщения менеджеру в Telegram.
